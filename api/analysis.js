...
async function runRasterCase(id) {
  ...
  for (let attempt = 0; attempt < 24; attempt += 1) {
    ...
    if (raw?.status !== "running" && raw?.status !== "notStarted") throw new Error(`Azure raster case ${id} failed with ${raw?.status || "unknown"}`);
    await sleep(1250);
  }

  if (raw?.status !== "succeeded") throw new Error(`Azure raster case ${id} timed out`);
  ...
  const qcCase = createPackageQcReview({ result, meta: { analysisId: batch.analysisId, modelId: batch.modelId, apiVersion: batch.apiVersion } });
  const predictedRecommendation = computeRecommendation(qcCase.rules);
  const score = scorePdfEvaluationCase({ scenario, result, predictedRecommendation, latencyMs: Date.now() - startedAt, provider: result.provider });
  ...
  return {
    ...
    fixture: { dpi: scenario.dpi, jpegQuality: scenario.jpegQuality, blurSigma: scenario.blurSigma, rotateDegrees: scenario.rotateDegrees, imageOnly: fixture.imageOnly },
  };
}
...
export default async function handler(request, response) {
  ...
  if (request.query.rasterBenchmark === "post-fix") {
    try {
      const first = await runRasterCase("RASTER-001");
      await sleep(12000);
      const second = await runRasterCase("RASTER-002");
      return send(response, 200, { status: "succeeded", cases: [first, second] });
    } catch (error) {
      console.error("Post-fix raster benchmark failed", error);
      return send(response, error.statusCode || 502, { error: error.message || "Post-fix raster benchmark failed" });
    }
  }
