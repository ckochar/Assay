...
export const PDF_STRESS_SCENARIOS = Object.freeze([
  Object.freeze({
    ...
    description: "Keeps the clean package content but rotates selected pages 90 or 270 degrees to test page-orientation recovery.",
  ...
  Object.freeze({
    ...
    description: "Uses smaller, lighter text and a denser two-column layout while preserving the same labeled package facts.",
    label: Object.freeze(baseLabel()),
    mode: "compact",
  }),
  Object.freeze({
    ...
    label: Object.freeze(baseLabel({
      ...
      fields: {
        ...baseLabel().fields,
        notaryAcknowledgmentDate: null,
        notaryCommissionExpirationDate: null,
      },
      evidencePages: {
        ...baseLabel().evidencePages,
        notaryAcknowledgmentDate: null,
        notaryCommissionExpirationDate: null,
      },
    })),
...
export function getPdfStressScenario(id) {
  return PDF_STRESS_SCENARIOS.find((scenario) => scenario.id === id) || null;
}

function drawStandardPage(page, spec, fonts) {
  page.drawText(spec.title, { x: 54, y: 730, size: 16, font: fonts.bold });
  spec.lines.forEach((line, index) => page.drawText(line, { x: 54, y: 690 - index * 24, size: 10.5, font: fonts.regular }));
}

function drawCompactPage(page, spec, fonts) {
  const ink = rgb(0.58, 0.58, 0.58);
  page.drawText(spec.title, { x: 44, y: 738, size: 11.5, font: fonts.bold, color: ink });
...
export async function createPdfStressFixture(id) {
  const scenario = getPdfStressScenario(id);
  if (!scenario) throw new Error(`Unknown PDF stress scenario: ${id}`);

  const pages = clone(BASE_PAGES);
  if (scenario.mode === "structure") pages[7] = clone(BASE_PAGES[3]);
  ...
  pages.forEach((spec, index) => {
    ...
    if (scenario.mode === "compact") drawCompactPage(page, spec, fonts);
    else drawStandardPage(page, spec, fonts);

    if (scenario.mode === "rotation") {
      if (index === 3 || index === 7) page.setRotation(degrees(90));
      if (index === 4) page.setRotation(degrees(270));
    }

    page.drawText(`Assay ${scenario.id} synthetic stress fixture. Not a legal instrument.`, {
      x: 44,
      y: 40,
      size: 8,
      font: fonts.italic,
