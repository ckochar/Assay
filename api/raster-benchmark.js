...
const ALLOWED = new Set(["RASTER-001", "RASTER-002"]);
  ...
  try {
    ...
    for (let attempt = 0; attempt < 24; attempt += 1) {
      ...
      if (raw?.status !== "running" && raw?.status !== "notStarted") {
        return send(response, 502, { error: "Azure raster benchmark analysis failed", status: raw?.status || "unknown" });
      }
      await sleep(1250);
    }

    if (raw?.status !== "succeeded") return send(response, 504, { error: "Raster benchmark timed out before Azure completed" });
    ...
    console.error("Raster benchmark failed", error);
    return send(response, error?.statusCode || 502, { error: error?.message || "Raster benchmark failed" });
  }
}
