...
function ExperienceMode() {
  return <div style={{ display: "grid", justifyItems: "end", gap: 4 }}>
    ...
    <div style={{ display: "flex", border: `1px solid ${C.line}`, borderRadius: 8, padding: 3, background: C.bg }}>
      <a href="/" style={{ ...mono, textDecoration: "none", fontSize: 10, fontWeight: 750, padding: "6px 9px", borderRadius: 6, color: C.sub }}>Demo Workspace</a>
      <a href="/live" style={{ ...mono, textDecoration: "none", fontSize: 10, fontWeight: 750, padding: "6px 9px", borderRadius: 6, color: C.teal, background: C.tealSoft }}>Live Analysis</a>
    </div>
    <div style={{ color: C.sub, fontSize: 10 }}>Live Azure pipeline · use sample documents only</div>
  </div>;
}
...
export default function LiveAnalysis() {
  ...
  const resetAnalysis = (nextFile) => {
    setFile(nextFile); setResult(null); setMeta(null); setActiveRuleId(null); setError(""); setPhase("idle");
  };
  ...
  const analyze = async () => {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 30; attempt += 1) {
        ...
        setResult(payload.result); setActiveRuleId(payload.result?.rules?.[0]?.id || null); setPhase("complete"); return;
      }
  ...
  return <main style={{ minHeight: "100vh", background: C.bg, color: C.ink, ...display }}>
    <header style={{ background: C.panel, borderBottom: `1px solid ${C.line}`, padding: "12px 22px", display: "flex", justifyContent: "space-between", gap: 16, alignItems: "center", flexWrap: "wrap" }}>
      <a href="/" style={{ display: "flex", gap: 10, alignItems: "center", color: C.ink, textDecoration: "none" }}><div style={{ width: 34, height: 34, display: "grid", placeItems: "center", borderRadius: 9, background: C.teal, color: "white", fontWeight: 800 }}>AY</div><div><b>Assay</b><div style={{ ...mono, color: C.sub, fontSize: 10 }}>post-execution mortgage QC</div></div></a>
      <ExperienceMode />
    </header>
    <div style={{ maxWidth: 1480, margin: "0 auto", padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "end", gap: 14, flexWrap: "wrap", marginBottom: 16 }}><div><h1 style={{ margin: 0, fontSize: 26 }}>Live Analysis</h1><p style={{ color: C.sub, marginBottom: 0 }}>Upload a sample Promissory Note and run the real Azure document-analysis pipeline.</p></div><Pill status="Needs Review">BETA · 2-PAGE SCOPE</Pill></div>

      <section style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 12, padding: 18 }}>
        <div style={{ background: C.blueSoft, color: C.blue, borderRadius: 8, padding: 11, fontSize: 12, marginBottom: 14 }}><b>Use sample documents only.</b> PDFs are sent directly to Azure Document Intelligence for analysis and are not persisted by Assay. This first live workflow reads pages 1–2 only.</div>
      ...
      {result && <>
        <section style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 10, margin: "16px 0" }}>{[["Document type", result.document.type], ["Loan number", result.document.loanNumber || "Not found"], ["Borrowers", result.document.borrowers.join("; ") || "Not found"], ["Execution date", result.document.executionDate || "Not found"], ["Pages analyzed", result.document.pageCount], ["OCR quality", result.document.ocrQuality.label]].map(([label, value]) => <div key={label} style={{ background: C.panel, border: `1px solid ${C.line}`, borderRadius: 10, padding: 13 }}><div style={{ ...mono, color: C.sub, fontSize: 9 }}>{label.toUpperCase()}</div><div style={{ fontWeight: 800, marginTop: 5, fontSize: 13 }}>{value}</div></div>)}</section>
        <section style={{ display: "grid", gridTemplateColumns: "minmax(440px,1.15fr) minmax(390px,.85fr)", gap: 14, alignItems: "start" }}>
          <div style={{ position: "sticky", top: 12 }}><PdfEvidenceViewer file={file} evidence={activeRule?.evidence} /></div>
          <div><div style={{ display: "flex", justifyContent: "space-between", gap: 10, alignItems: "center", marginBottom: 8 }}><div><b>QC findings</b><div style={{ color: C.sub, fontSize: 11 }}>Select a finding to inspect its source evidence.</div></div><span style={{ ...mono, color: C.sub, fontSize: 10 }}>{result.rules.length} controls</span></div>
            <section style={{ display: "grid", gap: 10 }}>{result.rules.map((rule) => { const selected = activeRule?.id === rule.id; return <article key={rule.id} style={{ background: C.panel, border: `1px solid ${selected ? tone(rule.status).color : `${tone(rule.status).color}55`}`, borderLeft: `4px solid ${tone(rule.status).color}`, boxShadow: selected ? `0 0 0 2px ${tone(rule.status).color}18` : "none", borderRadius: 10, padding: 14 }}><div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap" }}><div><span style={{ ...mono, fontSize: 10, color: C.sub }}>{rule.id}</span> · <b>{rule.name}</b></div><Pill status={rule.status}>{rule.status}</Pill></div><div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginTop: 10 }}><div style={{ background: C.bg, padding: 10, borderRadius: 8, fontSize: 11 }}><span style={{ color: C.sub }}>Extracted result</span><br /><b>{rule.extractedValue}</b></div><div style={{ background: C.bg, padding: 10, borderRadius: 8, fontSize: 11 }}><span style={{ color: C.sub }}>Evidence</span><br /><b>Page {rule.evidence.page}</b> · {rule.evidence.excerpt}</div></div><div style={{ ...mono, color: C.sub, fontSize: 10, marginTop: 9 }}>Classification {rule.confidence.classification?.toFixed(2) || "—"} · Extraction {rule.confidence.extraction?.toFixed(2) || "—"} · OCR {rule.confidence.ocrQuality} · Evidence {rule.confidence.evidenceComplete ? "complete" : "incomplete"}</div>{rule.confidence.reviewTrigger && <div style={{ color: C.review, fontSize: 11, marginTop: 7 }}>Human review trigger: {rule.confidence.reviewTrigger}</div>}<button type="button" onClick={() => setActiveRuleId(rule.id)} style={{ marginTop: 10, border: `1px solid ${C.line}`, background: selected ? C.tealSoft : C.panel, color: C.teal, borderRadius: 7, padding: "7px 10px", fontSize: 11, fontWeight: 800, cursor: "pointer" }}>{selected ? "Evidence selected" : `View evidence on page ${rule.evidence.page}`}</button></article>; })}</section>
          </div>
        </section>
      </>}
    </div>
