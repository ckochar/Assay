...
function removeLines(pages, matcher) { pages.forEach((page) => { page.lines = page.lines.filter((line) => !matcher.test(line)); }); }
...
function label({ expectedRecommendation, jurisdiction = "TX", loanNumber = "LN-900001", loanNumberCandidates = ["LN-900001"], pageTypes = null }) {
  return {
    ...
    fields: { loanNumber, loanNumberCandidates, borrowers: ["Maya Patel", "Rohan Patel"], jurisdiction, noteExecutionDate: "2026-08-09", closingDate: "2026-08-09", rtcTransactionDate: "2026-08-09", rtcCancellationDeadline: "2026-08-12", notaryAcknowledgmentDate: "2026-08-09", notaryCommissionExpirationDate: "2028-11-30" },
    evidencePages: { loanNumber: 1, borrowers: 1, jurisdiction: jurisdiction ? 1 : null, noteExecutionDate: 1, closingDate: 4, rtcTransactionDate: 5, rtcCancellationDeadline: 5, notaryAcknowledgmentDate: 8, notaryCommissionExpirationDate: 8 },
  };
}

export const PDF_EVALUATION_SCENARIOS = Object.freeze([
  { id: "PDF-001", name: "Clean machine-readable package", category: "Clean baseline", description: "Eight-page digital PDF with all target document types and internally consistent structured evidence.", label: label({ expectedRecommendation: "Needs Review" }), mutate() {} },
  { id: "PDF-002", name: "Unknown page in package", category: "Classification routing", description: "Replaces the occupancy affidavit with an unsupported executed form so the page should remain explicitly unknown.", label: label({ expectedRecommendation: "Needs Review", pageTypes: BASE_PAGES.map((page, index) => index === 5 ? "Unknown document" : page.type) }), mutate(pages) { pages[5] = { type: "Unknown document", title: "BORROWER DISCLOSURE", lines: ["Loan No.: LN-900001", "This executed disclosure is intentionally outside the prototype document taxonomy.", "Borrower: Maya Patel", "Borrower: Rohan Patel"] }; } },
  { id: "PDF-003", name: "Unresolved jurisdiction", category: "Context routing", description: "Removes supported property-state evidence and uses an unsupported notary venue so Assay must not invent a TX/CA/FL profile.", label: label({ expectedRecommendation: "Needs Review", jurisdiction: null }), mutate(pages) { removeLines(pages, /Property Address:/i); replaceLine(pages[7], /State of Texas/i, "State of Nevada"); } },
  { id: "PDF-004", name: "Conflicting loan identifiers", category: "Deterministic exception", description: "Closing Disclosure carries a different loan number from the rest of the package.", label: label({ expectedRecommendation: "Exception Identified", loanNumber: null, loanNumberCandidates: ["LN-900001", "LN-900002"] }), mutate(pages) { replaceLine(pages[3], /Loan No\.:/i, "Loan No.: LN-900002"); } },
  { id: "PDF-005", name: "Impossible date chronology", category: "Deterministic exception", description: "RTC deadline and notary commission expiration each precede their corresponding execution date.", label: { ...label({ expectedRecommendation: "Exception Identified" }), fields: { ...label({ expectedRecommendation: "Exception Identified" }).fields, rtcCancellationDeadline: "2026-08-08", notaryCommissionExpirationDate: "2026-08-08" } }, mutate(pages) { replaceLine(pages[4], /Cancellation Deadline:/i, "Cancellation Deadline: August 8, 2026"); replaceLine(pages[7], /My commission expires/i, "My commission expires August 8, 2026."); } },
].map((scenario) => Object.freeze({ ...scenario, label: Object.freeze(scenario.label) })));

export function getPdfEvaluationScenario(id) { return PDF_EVALUATION_SCENARIOS.find((scenario) => scenario.id === id) || null; }
...
export async function createPdfEvaluationFixture(id) {
  ...
  const pages = clone(BASE_PAGES); scenario.mutate(pages);
